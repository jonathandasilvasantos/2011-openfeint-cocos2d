...
- (void)didFailUpload:(OFCloudStorageStatus_Code)statusCode{
    ...
    if(statusCode != CSC_Ok)
    {
        NSLog(@"Failure delegate handles errors, never handles CSC_Ok.");
    }
...
- (IBAction)onBtnBlobSend:(id)sender{
	...
	OFRequestHandle* handle = [OFCloudStorage upload:payloadData
											 withKey:blobNameTextBox.text];
	
	if(!handle)
	{
		OFLog(@"Did not get a request handle back from OFCloudStorage's upload");
	}
}
...
- (IBAction)onBtnBlobFetch:(id)sender{
	...
	OFRequestHandle* handle = [OFCloudStorage downloadDataWithKey:blobNameTextBox.text];
	
	if(!handle)
	{
		OFLog(@"Did not get request handle from OFCloudSotrage's downloadDataWithKey:");
	}
}
...
- (IBAction) onBtnPrintAllCurrentUsersDataKeys:(id) sender
{
	...
	OFRequestHandle* handle = [OFCloudStorage downloadKeysForCurrentUser];
	
	if(!handle)
	{
		OFLog(@"Did not get request handle from OFCloutStorage's downloadKeysForCurrentUser");
	}
}

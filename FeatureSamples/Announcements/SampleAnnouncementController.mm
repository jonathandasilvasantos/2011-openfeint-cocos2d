...
- (IBAction) onBtnMarkAsRead:(id) sender
{
	if([curAnnouncementList count] > 0)
	{
		OFAnnouncement *announcement = (OFAnnouncement*)[curAnnouncementList objectAtIndex:announcementIndex];
		[announcement markAsRead];
		[self refreshStatusText:announcement];
	}
}
